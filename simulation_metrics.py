...
def metric(x: np.ndarray, metrics_all, metric_name):
    if 8 < len(x):
        domain_id = x[DOMAIN_ENCODE_ID]
        var = x[DOMAIN_ENCODE_ID + 1:]
    else:
        var = x
    version_to_match = var_to_version(var)
    metric = 99.99  # max
    for case in metrics_all:
        if case['version'] == version_to_match:
            metric = case[metric_name]
            break
    # print(f"call CPIMetric x: {x}, y: {metric}, version: {version_to_match}")
    if 99.98 < metric:
        print(f"missing: call metric-{metric_name} x: {x}, version: {version_to_match}")
    elif metric < 0.2:
        print(f"missing: call metric-{metric_name} x: {x}, version: {version_to_match}")
    return metric
...
def read_metrics_cases(config_dir):
    metrics_cases = {}
